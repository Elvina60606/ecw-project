...
const Orders = () => {
  ...
  const { orders, currentPage, pagination } = useSelector(
    (state) => state.orders,
  );
    ...
    <>
      <div className="col-md-9">
        <div className="tab-content">
          {/*--訂單紀錄內容--*/}
          <div className="tab-pane active" tabIndex="0">
            ...
            <div className="border border-1 rounded-16 p-3 fs-6 d-lg-none bg-white">
              {orders.map((order) => {
                return (
                  <div
                    className="order-info-mobile py-3 mb-6 border-bottom"
                    key={order.id}
                  >
                    <div>
                    ...
                    <div>
                      ...
                      {order.is_paid ? (
                        <span className="border border-0 rounded-pill bg-success-50 text-success-700 px-3 py-2">
                          已完成
                        </span>
                      ) : (
                        <span className="border border-0 rounded-pill bg-alert-50 text-alert-700 px-3 py-2">
                          已成立
                        </span>
                      )}
                    </div>
                    <div>
                      ...
                      <div className="d-flex flex-column align-items-end">
                        {Object.values(order.products || {}).map((product) => (
                          <p key={product.id}>
                            {product.product.title}({product.product.content}) x{" "}
                            {product.qty}
                          </p>
                        ))}
                      </div>
                          ...
                          {Object.values(order.products || {}).map(
                            (product) => (
                              <div key={product.id}>
                                ...
                                {product.product.content}) x {product.qty}
                              </div>
                            ),
                          )}
                        </td>
