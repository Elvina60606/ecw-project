...
const AdminOrderModal =() => {
    ...
    const totalPrice = orderProducts.reduce((sum, item) => sum + item.total * item.qty, 0);
        ...
        <div className="modal-overlay" tabIndex="-1">
            <div className="admin-modal-box" style={{width: '70%'}}>
                        ...
                        <div className="col-6">
                            ...
                            {orderProducts.map((item) => (
                                <div key={item.id} className="mb-4">
                                    ...
                                    <p>${item.total} x {item.qty} = ${item.total * item.qty}</p>
                ...
                <div className="modal-footer d-flex justify-content-end gap-2">
                    <button type="button" className="btn btn-primary" onClick={()=>dispatch(closeModal())}>關閉</button>
                </div>
            </div>
        </div>
    </>)
