  ...
  async (_, { dispatch }) => {
    try {
      const res = await axios.get(
        `${VITE_URL}/v2/api/${VITE_PATH}/admin/orders`,
      );
      return {
  ...
  async ({ id, data }, { dispatch }) => {
    try {
      const res = await axios.put(
        `${VITE_URL}/v2/api/${VITE_PATH}/admin/order/${id}`,
        { data },
      );
  ...
  async (orderId, { dispatch }) => {
    try {
      const res = await axios.delete(
        `${VITE_URL}/v2/api/${VITE_PATH}/admin/order/${orderId}`,
      );
      dispatch(getAsyncMessage(res.data));
      dispatch(getAsyncAdminOrders());
    } catch (error) {
...
export const adminOrdersSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder.addCase(getAsyncAdminOrders.fulfilled, (state, action) => {
