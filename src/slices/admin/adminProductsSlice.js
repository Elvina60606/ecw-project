  ...
  async (productId, { dispatch }) => {
    try {
      const res = await axios.delete(
        `${VITE_URL}/v2/api/${VITE_PATH}/admin/product/${productId}`,
      );
  ...
  async (data, { dispatch }) => {
    ...
    try {
      const res = await axios.post(
        `${VITE_URL}/v2/api/${VITE_PATH}/admin/product`,
        productData,
      );
      dispatch(getAsyncMessage(res.data));
    } catch (error) {
...
export const adminProductsSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder.addCase(getAsynsAdminProducts.fulfilled, (state, action) => {
...
export const { setCurrentPage, setTempProduct, resetTempProduct } =
